...
var renderPin = function (mainArr) {
  // копируем шаблон
...
clonedPin.style = 'left: ' + (mainArr[i].Data._location.x + 20) + 'px; top: '+ (mainArr[i].Data._location.y + 40) + 'px';
...
fragment.appendChild(renderPin(mainArr[i]))
// цикл закрываем
//добавляем фрагмент в map__pins
houseOfPins.appendChild(fragment);
//задание 5 
// запрос шаблона обьявления 
...
var renderCard = function (mainArr)
{
  // копируем шаблон
...
clonedCard.querySelector('.popup__text--capacity').textContent = mainArr[i].Data.offer.rooms + ' комнаты для ' + mainArr[i].Data.offer.guests + ' гостей'
clonedCard.querySelector('.popup__text--time').textContent = 'Заезд после ' + mainArr[i].Data.offer.checkin + ',выезд до ' + mainArr[i].Data.offer.checkout
clonedCard.querySelector('.popup__features') = mainArr[i].Data.offer.features
clonedCard.querySelector('.popup__photos').src = mainArr[i].Data.offer.photos[0]
...
var fragment = document.createDocumentFragment();
// цикл
fragment.appendChild(renderCard(mainArr[i]))
// конец цикла
...
blockMap.insertBefore(fragment, BlocMapFilters);
  ...
  var adressPhotos = document.querySelector('.popup__photo');
  var templatePhoto = document.querySelector('template').content.querySelector('.popup__photo');
  for (var j = 1; j < photos.length; j++) {
    var cloneSlotPhoto = templatePhoto.cloneNode(true);
    cloneSlotPhoto.src = mainArr[i].Data.offer.photos[j]
  }
